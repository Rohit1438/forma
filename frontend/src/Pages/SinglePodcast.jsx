...
const SinglePodcast = () => {
  ...
  const [ptojectTitle, setformTitle] = useState("");
  ...
  const fetchQuestions = async () => {
    console.log("questions fetching",id);
    try {
      let res = await axios.get(`${BASE_URL}/forms/questions/${id}`, {
  ...
  const uploadquestion = async () => {
    try {
      let res = await axios.post(
        `${BASE_URL}/forms/addquestion/${id}`,
        { title: questionName, description: questionDesc },
        { headers: { authorization: `Bearer ${token}` } }
      );
  ...
  const deletequestions = async (questionId) => {
    try {
      let res = await axios.delete(
        `${BASE_URL}/forms/deletequestion/${questionId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      let updatedquestions = await questions.filter((el) => {
        console.log(el);
        return el._id !== questionId;
      });
      // await  setquestions((pre)=>updatequestions)
  ...
  console.log(questions, "episdoes data and length", questions.length);

  const updatequestions = async (questionId) => {
    try {
      let res = await axios.put(
        `${BASE_URL}/forms/updatequestion/${questionId}`,
        { title: questionName, description: questionDesc },
        { headers: { authorization: `Bearer ${token}` } }
      );
      await fetchQuestions();
          ...
          <div>
            <div>
              <Breadcrumb>
                ...
                <BreadcrumbItem>
                  <BreadcrumbLink>{formName}</BreadcrumbLink>
                </BreadcrumbItem>
            ...
            >
              ...
              <div className="editbtns">
                <button
                  ...
                  onClick={() => setIsEditClicked((pre) => false)}
                >
                  Discard
                </button>
                <button
                  className="btn"
                  onClick={() => updatequestions(updateId)}
                >
                  Save & Exit
                </button>
              </div>
            ...
            <div className="transcriptArea">
              <Textarea
                ...
                value={questionDesc}
                onChange={(e) => setquestionDesc((pre) => e.target.value)}
                size="lg"
              />
            </div>
          ...
          <div>
            ...
            <div className="uploadOptions">
              ...
              >
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>Upload your question</AlertDialogHeader>
                  <AlertDialogCloseButton />
                  <AlertDialogBody>
                    <FormControl>
            ...
            >
              <p>All questions are processed! Your form is ready to go!</p>
              <button
                style={{
                  color: "#7E22CE",
                  fontWeight: "600",
                        ...
                        {/* <td>
                          <ButtonGroup size="sm" isAttached variant="outline">
                            <Button
                              color="whatsapp"
                              onClick={() => {
                                setquestionName((pre) => el.title);
                                setquestionDesc((pre) => el.description);
